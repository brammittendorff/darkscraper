...
static ONION_RE: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"[a-z2-7]{56}\.onion").unwrap());

static I2P_RE: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"[a-zA-Z0-9\-]+\.i2p").unwrap());
...
static ETH_RE: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"\b0x[a-fA-F0-9]{40}\b").unwrap());
...
static PGP_RE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(
        r"[0-9A-Fa-f]{4}[\s]?[0-9A-Fa-f]{4}[\s]?[0-9A-Fa-f]{4}[\s]?[0-9A-Fa-f]{4}[\s]?[0-9A-Fa-f]{4}[\s]?[0-9A-Fa-f]{4}[\s]?[0-9A-Fa-f]{4}[\s]?[0-9A-Fa-f]{4}[\s]?[0-9A-Fa-f]{4}[\s]?[0-9A-Fa-f]{4}",
    )
    .unwrap()
});

static USERNAME_RE: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"@[a-zA-Z0-9_]{1,50}\b").unwrap());
...
pub fn extract_entities(text: &str) -> ExtractedEntities {
    ...
    let pgp_fingerprints = find_unique(&PGP_RE, text);
